...
def get_body(page_xml):
    #returns xml body text
    #up to first 25k characters
    
    #extract article body
    tree = ET.fromstring(page_xml)
    try:
        for child in tree:
            if child.tag[-8:] == "revision":
                for j in child:
                    if j.tag[-4:] == "text":
                        body = j.text
        ...
        print "can't parse article body"
        return False
    return body[:25000] #returns body 
...
def clean_link(link):
    ...
    return link.lower() #lower case

def run_parser(page_xml):
    links = grab_links(get_body(page_xml))
    for link in links:
        if check_link(link):
