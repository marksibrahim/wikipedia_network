...
inventions, places, people, objects, and events are related and organized.  
We traverse every path, measuring the accumulation of first links, path lengths, basins, cycles, 
and the influence each article exerts in shaping the network.
We discover scale-free distributions describe path length, 
...
Philosophy shapes more paths than any other article by two orders of magnitude. 
Curiously, we also observe a gravitation towards topical articles such as 
...
elements in the side bars and headings---we 
systematically capture the core description of a topic.
...
For methodological details, see section~\ref{Appendix}. 
...
The in-degree 
provides a way to rank the articles in Wikipedia by the number of first link references.
...
For example ``Train'' has a first link to 
``Rail Transport,''
 which is itself
an article with a first link to 
...
\cite{geo_basins, dodds} and describe the organization of food systems through transportation networks
\cite{food_webs}.
...
googol possible paths ($4.7*10^6*2^{4.7*10^6}$) through the FLN .
...
In Fig.~\ref{fig:Distribution of Visits}, we show traversal visits against rank, yielding
an excellent fit with regimes corresponding to power law rank exponents of 
...
by the accumulation of first links.
...
The first links anchor the wealth of specific knowledge on Wikipedia to 
a few notions foundational to society. 
...
Of the 11 million articles, roughly 84,000 are memebers of 2-cycles. 
...
Moving beyon basins around ``Philosophy,'' we find other basins around 
...
with the highest number of traversal visits, the variation in popularity appears to decrease as the number of traversal visits increases. The greatest number of articles fall within roughly 1 million traversal visits and page views.}
...
PSD and CMD acknowledge support from NSF Big Data Grand $\#1447634$.
...
\begin{thebibliography}{1}
    ...
    \bibitem{vacc}  Vermont Advanced Computing Core, \url{http://www.uvm.edu/vacc}.
