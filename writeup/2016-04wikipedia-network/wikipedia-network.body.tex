...
In our sample network (see Fig.~\ref{fig:Centrality Measures}), node A is has betweenness centrality of 0.17 and eigenvalue centrality of 0.58 although node A is simply the recipient of paths directed by node C.
Eigenvector centrality even assigns the same centrality to nodes A, B, and C. In contrast, traversal funnels defines a path without a predetermined end node, following the progression of links to its culmination at the start of a cycle. Furthermore, traversal funnels provides an interpretable metric: a node's traversal funnels is the number of paths the node directs to the start of a cycle. 
