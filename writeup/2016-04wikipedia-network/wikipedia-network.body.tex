...
\begin{figure}[tp!]
  ...
  \caption{
    ...
    The table above compares standard centrality measures on the sample network against traversal funnels.
    }
...
To compute traversal funnels on a directed network, begin at each node: 1. Traverse the edge to the next node 2. Increment the next node's count by one 3. Repeat while the next node is unvisited. The resulting count is each node's number of traversal funnels: the number of paths the node directs up to the start of a cycle.
...
Betweenness centrality and eigenvalue centrality capture flow within the network more than a single link away. Unlike traversal funnels however, betweenness centrality and eigenvalue centrality reward nodes that simply happen to be in a cycle. In our sample network (see Fig.~\ref{fig:Centrality Measures}), node A is relatively central using both betweenness and eigenvector centrality although node A is simply the recipient of paths directed by node C. Eigenvector centrality even assigns the same centrality to nodes A, B, and C. In contrast, traversal funnels defines a path without a predetermined end node, following the progression of links to its culmination at the start of a cycle. Furthermore, traversal funnels provides an interpretable metric: a node's traversal funnels is the number of paths the node directs to the start of a cycle. 
Consequently, traversal funnels allow us to isolate the paths directed by node C without any spill over effect on other nodes in its cycle. 
...
For our exploration of the FLN, by considering the FLN not only as collection of directly linked pairs of articles, but
